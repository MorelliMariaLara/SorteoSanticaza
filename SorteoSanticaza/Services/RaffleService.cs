...
public class RaffleService
{
    ...
    public RaffleService(Db db) => _db = db;
    ...
    public OrderResult CreateOrder(PurchaseForm input)
    {
        ...
        var packRaffleId = packReader.GetInt32(packReader.GetOrdinal("raffle_id"));
        var chances = packReader.GetInt32(packReader.GetOrdinal("chances"));
        var price = packReader.GetInt64(packReader.GetOrdinal("price_cents"));
        var label = packReader.GetString(packReader.GetOrdinal("label"));
        packReader.Close();
        ...
        var age = (DateTime.UtcNow - birth.ToUniversalTime()).TotalDays / 365.25;
        if (age < 18)
        ...
        insert.Parameters.AddWithValue("@birthDate", birth.ToString("yyyy-MM-dd"));
    ...
    public CheckoutResult ConfirmPayment(string publicId, string? paymentRef = null)
    {
        ...
        var raffleId = reader.GetInt32(reader.GetOrdinal("raffle_id"));
        var chances = reader.GetInt32(reader.GetOrdinal("chances"));
        var amount = reader.GetInt64(reader.GetOrdinal("amount_cents"));
    ...
    private static List<int> AllocateTicketNumbers(SqliteConnection conn, int raffleId, int count)
    {
        ...
        using (var raffleCmd = conn.CreateCommand())
        {
            ...
            if (!r.Read()) throw new InvalidOperationException("Sorteo no encontrado.");
            total = r.GetInt32(0);
            start = r.GetInt32(1);
        }
        ...
        using (var usedCmd = conn.CreateCommand())
        {
            ...
            while (r.Read()) used.Add(r.GetInt32(0));
        }

        var available = new List<int>();
        for (var n = start; n < start + total; n++)
        {
            if (!used.Contains(n)) available.Add(n);
        }
